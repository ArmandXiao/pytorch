...
class FPGMSparsifier(BaseStructuredSparsifier):
    ...
    def __init__(
        ...
        if dim != 0:
            raise NotImplementedError(
                "Only dim=0 (filter pruning) is currently supported."
            )
    ...
    def update_mask(self, module, tensor_name, sparsity_level, dim, **kwargs):
        ...
        if dim < 0:
            raise ValueError("Dimension to prune must be non-negative.")
        elif dim >= tensor_weight.dim():
            raise ValueError(f"Invalide pruning dimension: {dim}.")

        mask = getattr(module.parametrizations, tensor_name)[0].mask
        if (
            mask.shape != tensor_weight.shape[dim]
        ):  # initialize mask with correct dimension
